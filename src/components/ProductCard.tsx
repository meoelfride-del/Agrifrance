...
import { usePreferences } from "@/src/context/PreferenceContext";
...
export function ProductCard({ product, priority = false }: { product: Product; priority?: boolean }) {
  ...
  const { favorites, compared, toggleFavorite, toggleCompare } = usePreferences();
  return <article className="product-card-new">
    ...
    <div className="product-card-body"><small>{product.categorie} · {product.marque}</small><h3><Link href={`/produits/${product.categorySlug}/${product.slug}`}>{product.nom}</Link></h3><p>{t("catalog.description")}</p>
      ...
      <div className="product-secondary-actions"><button type="button" aria-pressed={favorites.includes(product.id)} onClick={() => toggleFavorite(product.id)}>{favorites.includes(product.id) ? "♥ Favori" : "♡ Ajouter aux favoris"}</button><button type="button" aria-pressed={compared.includes(product.id)} onClick={() => toggleCompare(product.id)}>{compared.includes(product.id) ? "✓ Comparé" : "Comparer"}</button></div>
    </div>
  </article>;
}
