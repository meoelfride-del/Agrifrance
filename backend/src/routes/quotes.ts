...
quoteRouter.post("/",async(req:AuthRequest,res,next)=>{try{ const d=quoteSchema.parse(req.body); const result=await query<{id:string;reference:string}>(`INSERT INTO quotes (user_id,company_id,product_slug,company_name,contact_name,email,phone,surface_hectares,message,configuration,currency) VALUES (NULL,NULL,$1,$2,$3,$4,$5,$6,$7,$8,'EUR') RETURNING id,reference`,[d.productSlug,d.companyName,d.contactName,d.email,d.phone,d.surfaceHectares,d.message,JSON.stringify(d.configuration)]); res.status(201).json(result.rows[0]); }catch(e){next(e);} });
